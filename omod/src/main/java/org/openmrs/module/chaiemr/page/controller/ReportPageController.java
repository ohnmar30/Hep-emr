	...
	public void get(@RequestParam("reportUuid") String reportUuid,
		...
		if (isIndicator) { 
			...
			for (int i = 0; i < 6; ++i) {
				...
				startDateOptions.put(chaiUi.formatDateParam(d), pretty.format(d));
			}

			model.addAttribute("startDateOptions", startDateOptions);
			model.addAttribute("startDateSelected", startDate != null ? chaiUi.formatDateParam(startDate) : null);
