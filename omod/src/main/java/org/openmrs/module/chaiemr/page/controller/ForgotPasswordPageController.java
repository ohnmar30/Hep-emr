...
import org.openmrs.module.chaiemr.IPAccessSecurity;
import org.openmrs.module.chaiui.ChaiUiUtils;
import org.openmrs.module.chaiui.annotation.PublicPage;
...
public class ForgotPasswordPageController {
	...
	public String controller(PageModel model,
		...
		if (!StringUtils.isEmpty(username)) {
			return handleSubmission(model, username, secretAnswer, request, chaiUi);
		}

		return null;
	}

	private String handleSubmission(PageModel model, String username, String secretAnswer, HttpServletRequest request, ChaiUiUtils chaiUi) {
		...
		if (IPAccessSecurity.isLockedOut(ipAddress)) {
			chaiUi.notifyError(httpSession, "auth.forgotPassword.tooManyAttempts");
		} else {
			if (StringUtils.isEmpty(secretAnswer)) {
				...
				if (user == null) {
					...
					chaiUi.notifyError(httpSession, "auth.question.empty");
				...
				if (user == null || StringUtils.isEmpty(user.getSecretQuestion())) {
					...
					chaiUi.notifySuccess(httpSession, "auth.password.reset");
					Context.authenticate(username, randomPassword);
					httpSession.setAttribute("loginAttempts", 0);
					return "redirect:/" + EmrConstants.MODULE_ID + "/profile.page";
				} else {
					chaiUi.notifyError(httpSession, "auth.answer.invalid");
					model.addAttribute("secretQuestion", user.getSecretQuestion());
					IPAccessSecurity.registerFailedAccess(ipAddress);
				}
			}
