...
<div class="ke-page-content">
	...
	<div class="ke-panel-frame">
		<div class="ke-panel-heading">${ ui.message("chaiemr.admin.firstTimeSetup") }</div>

		<div class="ke-panel-content">
			<form id="settings-form" action="${ ui.pageLink("chaiemr", "admin/firstTimeSetup") }">
				<% if (defaultLocation) { %>
					<div class="ke-warning" style="margin-bottom: 5px">Ensure that all visits are closed before changing the default location</div>
				<% } %>

				<% fields.each { %>
				${ ui.includeFragment("chaiui", "widget/labeledField", it) }
				<% } %>
			</form>
		</div>

		<div class="ke-panel-controls">
			<button type="submit" form="settings-form"><img src="${ ui.resourceLink("chaiui", "images/glyphs/ok.png") }" /> Save Settings</button>
		</div>
	</div>
