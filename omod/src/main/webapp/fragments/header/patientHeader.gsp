<%
	ui.includeJavascript("chaiemr", "controllers/header.js")

	def closeChartUrl = config.closeChartUrl ?: appHomepageUrl
%>
<script type="text/javascript">
	function ke_closeChart() {
		chaiui.openConfirmDialog({ heading: 'Patient chart', message: 'Close this patient chart?', okCallback: function() {
...
<div class="ke-patientheader" ng-controller="PatientHeader" ng-init="init(${ currentApp ? ("'" + currentApp.id + "'") : "null" }, ${ patient.id })">
	<div style="float: left; width: 35%;">
		...
		<span class="ke-patient-age">{{ patient.age }} <small>(${ chaiui.formatPersonBirthdate(patient) })</small></span>
	</div>
	...
	<div style="float: right">
		<button class="ke-compact" title="Close this patient chart" onclick="ke_closeChart()"><img src="${ ui.resourceLink("chaiui", "images/glyphs/close.png") }"/> Close Dashboard</button>
	</div>
