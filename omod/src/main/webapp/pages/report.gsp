<%
	...
	def menuItems =  [
			[ iconProvider: "chaiui", icon: "buttons/report_generate.png", label: "Request report", onClick: "requestReport()" ],
			[ iconProvider: "chaiui", icon: "buttons/back.png", label: "Back to home", href: returnUrl ]
	]
%>
<script type="text/javascript">
	function requestReport() {
		chaiui.openPanelDialog({ templateId: 'request-dialog-template' });
	}

	function onReportRequest(params) {
		chaiui.updateController('ng-reportctrl', function (scope) {
...
<div class="ke-page-content" id="ng-reportctrl" ng-controller="ReportController" ng-init="init('${ currentApp.id }', '${ definition.uuid }')">
	...
	<div class="ke-panel-frame">
		...
		<div class="ke-panel-content">
			${ ui.includeFragment("chaiui", "widget/dataPoint", [ label: "Name", value: definition.name ]) }
			<% if (definition.description) { %>
			${ ui.includeFragment("chaiui", "widget/dataPoint", [ label: "Description", value: definition.description ]) }
			<% } %>
		</div>
	</div>

	${ ui.includeFragment("chaiemr", "report/reportQueue", [ allowCancel: false ]) }
					...
					<td style="text-align: right">
						<div ng-if="request.status == 'COMPLETED'">
							<a href="#" ng-click="viewReportData(request.id)">
								<img src="${ ui.resourceLink("chaiui", "images/glyphs/view.png") }" class="ke-glyph" /> View
							</a>
							&nbsp;&nbsp;
							<a href="#" ng-click="exportReportData(request.id, 'csv')">
								<img src="${ ui.resourceLink("chaiui", "images/glyphs/csv.png") }" class="ke-glyph" /> CSV
							</a>
							&nbsp;&nbsp;
							<% if (excelRenderable) { %>
							<a href="#" ng-click="exportReportData(request.id , 'excel')">
								<img src="${ ui.resourceLink("chaiui", "images/glyphs/excel.png") }" class="ke-glyph" /> Excel
							</a>
							<% } %>
						</div>
						<div ng-if="request.status == 'FAILED'">
							<a href="#" ng-click="viewReportError(request.id)">
								<img src="${ ui.resourceLink("chaiui", "images/glyphs/monitor.png") }" class="ke-glyph" /> Error
							</a>
						</div>
					</td>
				</tr>
...
<div id="request-dialog-template" title="Request Report" style="display: none">
	${ ui.includeFragment("chaiemr", "report/reportRequestForm", [ definition: definition, onRequestCallback: "onReportRequest" ]) }
</div>