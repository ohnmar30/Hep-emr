 ...
 */
public class ScreeningRegisterPatientFragmentController {
	...
	public class EditPatientForm extends AbstractWebForm {
		...
		//maintain
			...
			if (hepCEnrollEncTypePrev.isEmpty()) {
				...
				Concept conceptlaborder = Context.getConceptService().getConceptByUuid(Dictionary.lABORATORY_ORDER);
				if (hBsAg)
				{
					...
					oLaborderHBsAg.setConcept(conceptlaborder);
					oLaborderHBsAg.setEncounter(enHcvNew);
					oLaborderHBsAg.setObsDatetime(date);
					oLaborderHBsAg.setValueCoded(conceptHbsAg);
					Context.getObsService().saveObs(oLaborderHBsAg, "HBsAg Lab order for Screening Registration");
				}
				...
				if (antiHCV)
				{
					...
					oLaborderAntiHCV.setConcept(conceptlaborder);
					oLaborderAntiHCV.setEncounter(enHcvNew);
					oLaborderAntiHCV.setObsDatetime(date);
					oLaborderAntiHCV.setValueCoded(conceptAntiHCV);
					Context.getObsService().saveObs(oLaborderAntiHCV, "AntiHCV Lab order for Screening Registration");
					
				}
				
			}
			
			//EncounterType encounterTypeLab = Context.getEncounterService().getEncounterTypeByUuid(CommonMetadata._EncounterType.LAB_ORDERS); //lab order encounter
			//Form fomLabOrder = Context.getFormService().getFormByUuid(CommonMetadata._Form.LAB_ORDERS);
