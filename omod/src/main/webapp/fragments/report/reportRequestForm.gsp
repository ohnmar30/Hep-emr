...
<div class="ke-panel-content">
	<form id="${ config.id }">
		...
		<div class="ke-field-content">
			...
			${ ui.includeFragment("chaiemr", "field/reportPeriod", [ pastMonths: 60 ]) }
		</div>
		<% } %>
		<% params.each { name, param -> %>
		<div class="ke-field-label">${ param.label }</div>
		<div class="ke-field-content">
			${ ui.includeFragment("chaiui", "widget/field", [
...
<div class="ke-panel-footer">
	<button type="button" id="${ config.id }_btn"><img src="${ ui.resourceLink("chaiui", "images/glyphs/start.png") }" /> Request</button>
	<button type="button" onclick="chaiui.closeDialog();"><img src="${ ui.resourceLink("chaiui", "images/glyphs/cancel.png") }" /> Cancel</button>
</div>