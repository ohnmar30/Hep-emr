<%
	ui.decorateWith("chaiui", "panel", [ heading: "Step 2: Register Account" ])
...
<form id="create-account-form" method="post" action="${ ui.actionLink("chaiemr", "account/newAccount", "submit") }">
	...
	<div class="ke-form-footer">
		<button type="submit"><img src="${ ui.resourceLink("chaiui", "images/glyphs/ok.png") }" /> Create Account</button>
	</div>
</form>

<script type="text/javascript">
	jq(function() {
		chaiui.setupAjaxPost('create-account-form', {
			onSuccess: function(data) {
				if (data.personId) {
					ui.navigate('chaiemr', 'admin/manageAccounts');
				} else {
					chaiui.notifyError('Creating user was successful, but unexpected response');
				}
			}
		});
	});
