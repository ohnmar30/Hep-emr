	...
	public void controller(@FragmentParam(value="patient") Patient patient,
		...
		for (Relationship relationship : Context.getPersonService().getRelationshipsByPerson(patient)) {
			...
			if (person.isPatient()) {
				...
				linkIcon = ui.resourceLink("chaiui", "images/glyphs/patient_" + genderCode + ".png");
			}
			else {
				Map<String, Object> params = new HashMap<String, Object>();
				params.put("personId", person.getId());
				linkUrl = ui.pageLink(EmrConstants.MODULE_ID, "admin/editAccount", params);
				linkIcon = ui.resourceLink("chaiui", "images/glyphs/person_" + genderCode + ".png");
			}

			Link link = new Link(chaiUi.formatPersonName(person), linkUrl, linkIcon);
