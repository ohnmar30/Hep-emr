...
chaiemrApp.controller('ReportController', ['$scope', '$http', '$timeout', function($scope, $http, $timeout) {
	...
	$scope.refresh = function(oneoff) {
		$http.get(ui.fragmentActionLink('chaiemr', 'report/reportUtils', 'getQueuedRequests', { reportUuid: $scope.reportUuid })).
			success(function(data) {
				$scope.queued = data;
			});

		$http.get(ui.fragmentActionLink('chaiemr', 'report/reportUtils', 'getFinishedRequests', { reportUuid: $scope.reportUuid })).
			success(function(data) {
				$scope.finished = data;
				if (!oneoff) {
	...
	$scope.requestReport = function(reportParams) {
		...
		$http.post(ui.fragmentActionLink('chaiemr', 'report/reportUtils', 'requestReport', params))
	...
	$scope.cancelRequest = function(requestId) {
		chaiui.openConfirmDialog({
			...
			okCallback: function() {
				$http.post(ui.fragmentActionLink('chaiemr', 'report/reportUtils', 'cancelRequest', { requestId: requestId }))
					.success(defaultSuccessHandler)
					.error(defaultErrorHandler);
			}
		});
	...
	$scope.viewReportData = function(requestId) {
		ui.navigate('chaiemr', 'reportView', { appId: $scope.appId, request: requestId, returnUrl: location.href });
	};
	...
	$scope.exportReportData = function(requestId, type) {
		ui.navigate('chaiemr', 'reportExport', { appId: $scope.appId, request: requestId, type: type });
	};
	...
	$scope.viewReportError = function(requestId) {
		var contentUrl = ui.pageLink('chaiemr', 'dialog/reportErrorDialog', { appId: $scope.appId, request: requestId });
		chaiui.openDynamicDialog({ heading: 'View Error', url: contentUrl, width: 90, height: 90 });
	};

	var defaultSuccessHandler = function(data) {
		chaiui.notifySuccess(data.message);
		$scope.refresh(true);
	};

	var defaultErrorHandler = function(data) {
		chaiui.notifyError(data.message);
	};
